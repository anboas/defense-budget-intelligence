...
function EventsView({ events, records, categories, canManageCategories, canCurateDiscovery, onAdd, onEdit, onDelete, onManageCategories, focusedEventId = "", embedded = false }) {
  const [researchEvent, setResearchEvent] = useState(null);
  const byId = new Map(records.map((record) => [record.opportunityId, record]));
  const columns = [
    { key: "title", label: "Event", required: true, sticky: true, minWidth: 260, value: (event) => event.title, searchValue: (event) => [event.title, event.notes, event.location, event.intelligence?.sponsor, event.intelligence?.branch, ...(event.intelligence?.topics || []), ...(event.links || []).flatMap((link) => [link.label, link.url]), ...eventCategoryLabels(event, categories), ...(event.milestones || []).flatMap((milestone) => [milestoneLabel(milestone), milestone.notes])], render: (event) => <><strong>{event.title}</strong><small><EventLocationLink location={event.location} fallback="Location not set" />{event.catalogEventId ? <span> · Catalog</span> : null}</small></> },
  ...
  const actions = <><a className="if-btn if-btn--secondary" href="#/budget-spend/tasks" aria-label="Task Center" title="Task Center"><ListChecks size={15} aria-hidden="true" /><span>Task Center</span></a>{canCurateDiscovery ? <a className="if-btn if-btn--secondary" href="#/budget-spend/event-discovery" aria-label="Manage event discovery" title="Manage event discovery"><Bot size={15} aria-hidden="true" /><span>Discovery</span></a> : null}{canManageCategories ? <button type="button" className="if-btn if-btn--secondary" aria-label="Manage categories" title="Manage categories" onClick={onManageCategories}><Tags size={15} aria-hidden="true" /><span>Manage categories</span></button> : null}<button type="button" className="if-btn if-btn--primary" aria-label="Add event" title="Add event" onClick={onAdd}><Plus size={15} aria-hidden="true" /><span>Add event</span></button></>;
...
function ScheduleView({ state, records, watchedRecords, categories, teams, auth, dataset, onAdd, onEdit, onDelete, onManageCategories }) {
  ...
  return <section className="ops-panel schedule-surface" data-schedule-surface={surface}>
    ...
    <ControlPageBody compact>
      {surface === "list" ? <EventsView embedded events={state.events} records={watchedRecords} categories={categories} canManageCategories={Boolean(auth?.user?.canManageWorkspace)} canCurateDiscovery={auth?.user?.roleId === "super_user" && !auth?.user?.isEmulating} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} onManageCategories={onManageCategories} focusedEventId={focusedEventId} /> : null}
