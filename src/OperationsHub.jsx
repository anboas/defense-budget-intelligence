...
import { ControlChangeList, ControlDialog, ControlDisclosure, ControlMetricStrip, ControlMultiSelect, ControlPageBody, ControlPageHeader, ControlProgressRail, ControlSparkline } from "control-surface-ui/react";
...
function EventAiReview({ jobId, onOpenDraft, activityEntries = [] }) {
  ...
  return <section className="if-operations-workspace" data-event-ai-review={job?.status || "loading"}>
    ...
    {panel === "review" && ["completed", "needs_review"].includes(job?.status) ? <>
      ...
      <section className="if-analytics-panel" data-event-ai-diff-preview><header className="if-analytics-panel__header"><div className="if-analytics-panel__heading"><h3 className="if-analytics-panel__title">Verified changes</h3><p className="if-analytics-panel__summary">Each changed field is shown once. The draft remains unsaved.</p></div><span className="if-badge if-badge--info">{diffRows.length} change{diffRows.length === 1 ? "" : "s"}</span></header><ControlChangeList label="Event draft difference preview" items={diffRows} empty={<div className="if-empty-state"><CircleCheck size={20} /><strong>No field differences</strong><p>The verifier did not produce a safe change to the original draft.</p></div>} />{job.mergeResult?.conflicts?.length ? <ControlDisclosure title={`Preserved operator values (${job.mergeResult.conflicts.length})`} summary="Conflicting operator-entered fields remain unchanged" tone="warning"><ul>{job.mergeResult.conflicts.map((conflict, index) => <li key={`${conflict.field || "field"}-${index}`}>{conflict.field || String(conflict)}</li>)}</ul></ControlDisclosure> : null}</section>
      <section className="if-analytics-panel"><header className="if-analytics-panel__header"><div className="if-analytics-panel__heading"><h3 className="if-analytics-panel__title">Evidence &amp; exclusions</h3><p className="if-analytics-panel__summary">Open cited sources or inspect claims excluded by verification.</p></div></header><div className="if-action-row-list">{(job.proposal?.sources || []).map((source, index) => <a key={source.url} className="if-action-row" href={source.url} target="_blank" rel="noreferrer"><span className="if-icon-slot"><Link2 size={15} /></span><span><strong>{source.publisher || source.title || `Source ${index + 1}`}</strong><em>{source.url}</em></span><span className="if-badge if-badge--info">Source</span></a>)}</div>{job.verification?.rejectedClaims?.length ? <ControlDisclosure title={`Rejected claims (${job.verification.rejectedClaims.length})`} summary="Claims excluded because verification could not support them"><ul>{job.verification.rejectedClaims.map((claim, index) => <li key={index}>{typeof claim === "string" ? claim : claim.reason || claim.claim || "Unsupported claim"}</li>)}</ul></ControlDisclosure> : null}</section>
