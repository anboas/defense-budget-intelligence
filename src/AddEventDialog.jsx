...
import { Bot, RotateCw, Search, Sparkles } from "lucide-react";
...
export default function AddEventDialog({ catalog, existingEvents, categories, teams, onCreate, onAugment, onClose }) {
  ...
  const footer = mode === "manual"
    ? <><button type="button" className="if-btn" disabled={Boolean(busyId)} onClick={onClose}>Cancel</button><button type="button" className="if-btn if-btn--secondary" disabled={Boolean(busyId)} onClick={researchManual}><Sparkles size={15} aria-hidden="true" />Research details</button><button type="button" className="if-btn if-btn--secondary" disabled={Boolean(busyId)} onClick={() => void saveManual(false)}>{busyId === "manual-save" ? workingSpinner() : null}Save event</button><button type="button" className="if-btn if-btn--ai" disabled={Boolean(busyId)} onClick={() => void saveManual(true)}>{busyId === "manual-augment" ? workingSpinner() : <Sparkles size={15} aria-hidden="true" />}Save &amp; augment</button></>
    : mode === "discovery"
  ...
  return <ControlDialog open onClose={() => { if (!busyId) onClose(); }} title="Add event" eyebrow="Workspace schedule" summary="Add a source-backed catalog event or start with your own event and let AI research the missing public details." size="wide" dialogRef={dialogRef} closeLabel="Close add event" surfaceProps={{ "data-add-event-dialog": mode }} footer={footer}>
    <nav className="if-tabs__list" aria-label="Add event method">
      <button type="button" className={`if-tab${mode === "catalog" ? " is-active" : ""}`} aria-pressed={mode === "catalog"} onClick={() => setMode("catalog")}>Search catalog</button>
      <button type="button" className={`if-tab${mode === "manual" ? " is-active" : ""}`} aria-pressed={mode === "manual"} onClick={() => setMode("manual")}>Add manually</button>
      {canCurate ? <button type="button" className={`if-tab${mode === "discovery" ? " is-active" : ""}`} aria-pressed={mode === "discovery"} onClick={() => { setMode("discovery"); if (!discovery) void loadDiscovery(); }}>Discovery review{discovery?.counts?.pending ? ` (${discovery.counts.pending})` : ""}</button> : null}
    </nav>
    ...
    {mode === "catalog" ? <div className="event-catalog-browser">
      <div className="event-catalog-browser__controls">
        <label className="if-field event-catalog-browser__search"><span className="if-field__label">Search curated events</span><span className="if-input-with-icon"><Search size={16} aria-hidden="true" /><input className="if-input" autoFocus value={query} placeholder="SOF Week, Army, autonomy, Orlando…" onChange={(event) => setQuery(event.target.value)} /></span></label>
      ...
      <label className="if-checkbox event-catalog-browser__wallboard"><input type="checkbox" checked={wallboard} onChange={(event) => setWallboard(event.target.checked)} /><span><strong>Show added events on wallboard</strong><small>Workspace and team visibility remain operator-owned.</small></span></label>
      <p className="if-field__hint">{results.length} of {catalog.length} curated events. Official sources are preferred; projected editions stay marked until dates are verified.</p>
      <div className="event-catalog-results" role="list" aria-label="Curated event catalog">{results.map((event) => {
        const existing = added.get(event.id);
        return <article key={event.id} className="event-catalog-card" role="listitem" data-catalog-event={event.id}><header><span><strong>{event.title}</strong><small>{event.sponsor} · {event.branch}</small></span><ControlStatusBadge status={event.confidence === "high" ? "verified" : event.confidence === "medium" ? "review" : "warning"} label={`${event.confidence} confidence`} /></header><p>{event.summary}</p><div className="event-catalog-card__meta"><span>{event.startsAt ? `${compactDate(event.startsAt)}${event.endsAt ? ` – ${compactDate(event.endsAt)}` : ""}` : "Date needs verification"}</span><span>{event.location || "Location pending"}</span><span>{event.eventType.replaceAll("_", " ")} · {event.format.replaceAll("_", " ")}</span></div><div className="event-catalog-card__topics">{(event.topics || []).slice(0, 4).map((topic) => <span key={topic} className="if-badge if-badge--neutral">{topic}</span>)}</div>{event.caveats?.length ? <small className="event-catalog-card__caveat">{event.caveats[0]}</small> : null}<footer>{event.links?.[0] ? <a href={event.links[0].url} target="_blank" rel="noreferrer">Official source</a> : <span>No official link retained</span>}<button type="button" className="if-btn if-btn--primary if-btn--sm" disabled={Boolean(existing) || Boolean(busyId)} onClick={() => void addCatalogEvent(event)}>{existing ? "Added" : busyId === event.id ? "Adding…" : event.startsAt ? "Add to calendar" : "Complete details"}</button></footer></article>;
      })}</div>
      ...
      {busyId === "discovery-load" ? <ControlAsyncState compact state="loading" title="Loading discovery queue" message="Reading source-backed candidates." /> : discovery?.candidates?.length ? <div className="event-catalog-results" role="list" aria-label="Event discovery candidates">{discovery.candidates.map((entry) => <article key={entry.id} className="event-catalog-card" role="listitem" data-event-discovery-candidate={entry.id}><header><span><strong>{entry.candidate.title}</strong><small>{entry.candidate.sponsor || entry.sourceId} · {entry.candidate.branch || "Joint"}</small></span><ControlStatusBadge status="review" label="Needs review" /></header><p>{entry.candidate.summary || "No summary was published in the official structured source."}</p><div className="event-catalog-card__meta"><span>{compactDate(entry.candidate.startsAt)}{entry.candidate.endsAt ? ` – ${compactDate(entry.candidate.endsAt)}` : ""}</span><span>{entry.candidate.location || "Location not published"}</span><span>Discovered {compactDate(entry.discoveredAt)}</span></div><footer><a href={entry.sourceUrl} target="_blank" rel="noreferrer">Inspect official source</a><span><button type="button" className="if-btn if-btn--secondary if-btn--sm" disabled={Boolean(busyId)} onClick={() => void reviewDiscovery(entry.id, "reject")}>Reject</button> <button type="button" className="if-btn if-btn--primary if-btn--sm" disabled={Boolean(busyId)} onClick={() => void reviewDiscovery(entry.id, "publish")}>{busyId === entry.id ? "Reviewing…" : "Publish to catalog"}</button></span></footer></article>)}</div> : <ControlAsyncState compact state="empty" icon={<Search size={22} />} title="No pending candidates" message="The queue is clear. Scan the next source pair now or wait for the hourly schedule." />}
