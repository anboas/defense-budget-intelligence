...
import { ChevronDown, ChevronLeft, ChevronRight, Columns3, Download, GripVertical, Search, X } from "lucide-react";
...
export default function OperationalDataTable({
  ...
  const defaults = useMemo(() => ({ density: "compact", visible: columnKeys, order: columnKeys, widths: {}, pageSize: defaultPageSize }), [columnKeys, defaultPageSize]);
  ...
  const pageSize = pageSizeOptions.includes(Number(preferences.pageSize)) ? Number(preferences.pageSize) : defaultPageSize;

  useEffect(() => {
    if (!id || typeof window === "undefined") return;
    window.localStorage.setItem(`dbi:data-table:${id}`, JSON.stringify({ density, visible: visibleKeys, order: orderedKeys, widths, pageSize }));
  }, [density, id, orderedKeys, pageSize, visibleKeys, widths]);
  ...
  return (
    ...
    >
      <div className="if-table-toolbar dbi-data-table__toolbar">
        ...
        <label className="if-search dbi-data-table__search">
        ...
        <div className="dbi-data-table__tools">
          {toolbarActions}
          <label className="dbi-data-table__density"><span className="sr-only">Table density</span><select className="if-select" value={density} onChange={(event) => { setPreferences((current) => ({ ...current, density: event.target.value })); setPage(1); }}>{Object.entries(DENSITIES).map(([value, text]) => <option value={value} key={value}>{text}</option>)}</select></label>
          <details className="dbi-data-table__columns"><summary className="if-btn if-btn--secondary"><Columns3 size={15} /><span>Columns</span><ChevronDown size={13} /></summary><div>{orderedKeys.map((key, index) => { const column = columnByKey.get(key); return <div className="dbi-data-table__column-option" key={key}><label><input type="checkbox" checked={visibleKeys.includes(key)} disabled={column.required} onChange={() => toggleColumn(key)} /><span>{column.label}</span></label><span className="dbi-data-table__column-order"><button type="button" onClick={() => moveColumn(key, -1)} disabled={index === 0} aria-label={`Move ${column.label} left`}><ChevronLeft size={13} /></button><button type="button" onClick={() => moveColumn(key, 1)} disabled={index === orderedKeys.length - 1} aria-label={`Move ${column.label} right`}><ChevronRight size={13} /></button></span></div>; })}</div></details>
      ...
      {facetColumns.length ? <div className="dbi-data-table__filters" data-table-filters>{facetColumns.map((column) => <label key={column.key}><span>{column.label}</span><select className="if-select" value={filters[column.key] || ""} onChange={(event) => { setFilters((current) => ({ ...current, [column.key]: event.target.value })); setPage(1); }}><option value="">All</option>{facetOptions[column.key].map((value) => <option key={value} value={value}>{value}</option>)}</select></label>)}{activeFilterCount ? <button type="button" className="if-btn if-btn--secondary" onClick={() => { setFilters({}); setPage(1); }}>Clear {activeFilterCount}</button> : null}</div> : null}
      ...
      <div className="if-table-wrap dbi-data-table__wrap">
        <table className={`if-table if-table--${density}`} aria-label={label}>
          ...
          <tbody>{pageRows.length ? pageRows.flatMap((row, index) => {
            ...
            >{selectable ? <td className="dbi-data-table__select" data-ui-table-card-label="Select"><input type="checkbox" checked={selected.has(key)} onChange={() => toggleRowSelection(key)} aria-label={`Select ${searchableValue(columns[0].value?.(row) ?? row[columns[0].key])}`} /></td> : null}{cells}</tr>, renderDetail && expanded ? <tr key={`${key}-detail`} className="if-table-detail" data-if-table-detail><td colSpan={visibleColumns.length + (selectable ? 1 : 0)}>{renderDetail(row)}</td></tr> : null];
      ...
      <footer className="if-table-footer dbi-data-table__footer"><span>Showing <strong>{filteredRows.length ? start + 1 : 0}</strong>–<strong>{Math.min(start + pageSize, filteredRows.length)}</strong> of <strong>{filteredRows.length}</strong></span><nav className="if-pagination" aria-label={`${label} pagination`}><button className="if-page-btn" type="button" onClick={() => setPage((value) => Math.max(1, value - 1))} disabled={safePage <= 1} aria-label="Previous table page"><ChevronLeft size={15} /></button><span>Page {safePage} of {pages}</span><button className="if-page-btn" type="button" onClick={() => setPage((value) => Math.min(pages, value + 1))} disabled={safePage >= pages} aria-label="Next table page"><ChevronRight size={15} /></button><select className="if-select" value={pageSize} onChange={(event) => setPreferences((current) => ({ ...current, pageSize: Number(event.target.value) }))} aria-label="Rows per page">{pageSizeOptions.map((value) => <option key={value} value={value}>{value} / page</option>)}</select></nav></footer>
    </section>
  );
}
