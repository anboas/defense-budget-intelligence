    ...
    >
      <div className="if-table-toolbar dbi-data-table__toolbar">
        ...
        <div className="dbi-data-table__tools">
          {showFacets && primaryFacet ? <div className="dbi-data-table__primary-filter"><span className="sr-only">{primaryFacet.label}</span><ControlSelect compact ariaLabel={`${primaryFacet.label} filter`} value={filters[primaryFacet.key] || ""} options={[["", `All ${primaryFacet.label.toLowerCase()}`], ...facetOptions[primaryFacet.key].map((value) => [value, value])]} onChange={(nextValue) => { setFilters((current) => ({ ...current, [primaryFacet.key]: nextValue })); setPage(1); }} /></div> : null}
          {toolbarActions}
