...
export default function OperationalDataTable({
  ...
  const primaryFacet = facetColumns[0] || null;
  const secondaryFacets = facetColumns.slice(1);
    ...
    >
      <div className="if-table-toolbar dbi-data-table__toolbar">
        ...
        <div className="dbi-data-table__tools">
          {showFacets && primaryFacet ? <div className="dbi-data-table__primary-filter"><span className="sr-only">{primaryFacet.label}</span><ControlSelect compact ariaLabel={`${primaryFacet.label} filter`} value={filters[primaryFacet.key] || ""} options={[["", primaryFacet.facetAllLabel || `All ${primaryFacet.label.toLowerCase()}`], ...facetOptions[primaryFacet.key].map((value) => [value, value])]} onChange={(nextValue) => { setFilters((current) => ({ ...current, [primaryFacet.key]: nextValue })); setPage(1); }} /></div> : null}
          {toolbarActions}
          <details ref={columnsRef} className="dbi-data-table__columns dbi-data-table__more"><summary className="if-btn if-btn--secondary"><MoreHorizontal size={15} aria-hidden="true" /><span>More</span><ChevronDown size={13} /></summary><div><header><strong>Table options</strong><button type="button" onClick={resetLayout}><RotateCcw size={13} />Reset layout</button></header><label className="dbi-data-table__more-density"><span>Density</span><ControlSelect compact ariaLabel="Table density" value={density} options={Object.entries(DENSITIES)} onChange={(nextDensity) => { setPreferences((current) => ({ ...current, density: nextDensity })); setPage(1); }} /></label><button className="if-btn if-btn--secondary dbi-data-table__more-export" type="button" onClick={() => downloadCsv(exportFilename, visibleColumns, selectedRows.length ? selectedRows : sortedRows)}><Download size={15} /><span>Export{selectedRows.length ? ` ${selectedRows.length}` : ""}</span></button><div className="dbi-data-table__more-heading"><Columns3 size={14} /><strong>Columns</strong></div>{orderedKeys.map((key, index) => { const column = columnByKey.get(key); return <div className="dbi-data-table__column-option" key={key}><label><input type="checkbox" checked={visibleKeys.includes(key)} disabled={column.required} onChange={() => toggleColumn(key)} /><span>{column.label}</span></label>{column.role === "actions" ? null : <span className="dbi-data-table__column-order"><button type="button" onClick={() => moveColumn(key, -1)} disabled={index === 0} aria-label={`Move ${column.label} left`}><ChevronLeft size={13} /></button><button type="button" onClick={() => moveColumn(key, 1)} disabled={index === orderedKeys.length - 1} aria-label={`Move ${column.label} right`}><ChevronRight size={13} /></button></span>}</div>; })}</div></details>
        </div>
      </div>
      {showFacets && secondaryFacets.length ? <>
      ...
      {filteredRows.length ? <footer className="if-table-footer dbi-data-table__footer"><span>Showing <strong>{start + 1}</strong>–<strong>{Math.min(start + pageSize, filteredRows.length)}</strong> of <strong>{filteredRows.length}</strong></span><nav className="if-pagination" aria-label={`${label} pagination`}><button className="if-page-btn dbi-page-edge" type="button" onClick={() => setPage(1)} disabled={safePage <= 1} aria-label="First table page"><ChevronsLeft size={15} /></button><button className="if-page-btn" type="button" onClick={() => setPage((value) => Math.max(1, value - 1))} disabled={safePage <= 1} aria-label="Previous table page"><ChevronLeft size={15} /></button><span>Page {safePage} of {pages}</span><button className="if-page-btn" type="button" onClick={() => setPage((value) => Math.min(pages, value + 1))} disabled={safePage >= pages} aria-label="Next table page"><ChevronRight size={15} /></button><button className="if-page-btn dbi-page-edge" type="button" onClick={() => setPage(pages)} disabled={safePage >= pages} aria-label="Last table page"><ChevronsRight size={15} /></button><ControlSelect compact className="dbi-data-table__page-size" value={String(pageSize)} options={availablePageSizes.map((value) => [String(value), `${value} / page`])} onChange={(nextValue) => { const key = compactTable ? "mobilePageSize" : "pageSize"; setPreferences((current) => ({ ...current, [key]: Number(nextValue) })); setPage(1); }} ariaLabel="Rows per page" /></nav></footer> : null}
    </section>
  );
}
