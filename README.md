...
SAM.gov refresh uses `npm run source:sam` to retrieve a rolling 180-day Department of Defense opportunity window when a protected `SAM_GOV_API_KEY` is available. Without the credential, the prior snapshot is preserved and the feed is explicitly labeled unavailable. The daily acquisition monitor runs at 09:32 UTC, records new/updated/removed source observations, retains first-seen and last-changed dates, and preserves 180 days of compact history. Manual and CRM-derived public records use the separate schema described in [Procurement ingestion and classification](docs/procurement-ingestion.md); private capture fields never enter the public bundle.
...
GitHub Actions checks active contracts, SAM notices, and the acquisition change ledger every day. The broader budget, execution, and source-health refresh remains weekly because budget books are annual source material and the retained award sample is not yet exhaustive. The site labels each layer independently as current, review-needed, or unavailable rather than presenting one misleading global freshness date.
...
- Explorer controls: technology-area sorting/filtering, Department of War → Military Departments/Fourth Estate → service or agency → buying-office drilldown, Date added sorting, and recoverable workspace tombstones are live. `Date added` means first retained DBI observation; source posting dates remain separate.
- Next ingest queue: historical C-1 discovery, service-hosted RDT&E/procurement justification books, full paginated USAspending award coverage, automated agency acquisition forecasts and DoD announcements, complete subaward-detail pagination, and exact predecessor/successor crosswalks.
