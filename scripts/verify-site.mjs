...
try {
  ...
  assert.equal(await page.locator("[data-capture-metric]").count(), 5, "Capture Calendar should expose five filtered metrics");
  assert.equal(await page.locator("[data-capture-chart]").count(), 8, "Capture Calendar should expose eight linked charts");
  ...
  assert.equal(await mobile.locator("[data-capture-chart]").count(), 8, "Mobile Capture Calendar should expose all linked charts");
  const captureTargets = await mobile.locator("[data-capture-filters] input, [data-capture-filters] select, [data-capture-filters] button, .capture-hero__actions button").evaluateAll((elements) => elements.map((element) => element.getBoundingClientRect().height));
  assert.ok(captureTargets.every((height) => height >= 43.5), `Mobile Capture Calendar controls should be 44px: ${captureTargets.join(", ")}`);
  const captureScroller = await mobile.locator("[data-capture-timeline]").evaluate((element) => ({ clientWidth: element.clientWidth, scrollWidth: element.scrollWidth }));
