...
try {
  ...
  assert.doesNotMatch(captureText, /OUR AWARD \/ DELIVERY-LED EXPANSION|ACTIVE TEAMED BID|Targets A1/i, "Capture Calendar should not expose internal campaign labels");
  assert.equal(await page.locator("[data-capture-filters]").count(), 1, "Capture Calendar should expose linked filters");
  assert.equal(await page.locator("[data-capture-metric]").count(), 4, "Capture Calendar should expose four filtered metrics");
  assert.equal(await page.locator("[data-capture-chart]").count(), 6, "Capture Calendar should expose six linked charts");
  ...
  assert.equal(await mobile.locator("[data-capture-chart]").count(), 6, "Mobile Capture Calendar should expose all linked charts");
