...
try {
  ...
  assert.equal(await page.locator("[data-active-page-title]").innerText(), "Queue", "Header should track Queue route title");
  assert.equal(await page.locator("[data-budget-filter-bar]").count(), 0, "Queue should not render budget line filters");
  const queueText = await page.locator("[data-defense-budget-app]").innerText();
  assert.match(queueText, /What To Validate Next/i);
  assert.match(queueText, /Capture Queue/i);
  assert.match(queueText, /Validation Backlog/i);
  assert.match(queueText, /Validate live SAM\.gov notices/i);
  assert.equal(await page.locator("[data-capture-filter-bar]").count(), 1, "Queue should expose capture-specific filters");
  ...
  await mobile.getByRole("button", { name: /^Queue$/ }).click();
  assert.equal(await mobile.locator("[data-budget-filter-bar]").count(), 0, "Mobile Queue should not render budget line filters");
  assert.match(await mobile.locator("[data-defense-budget-app]").innerText(), /What To Validate Next/i);
  assert.equal(await mobile.locator("[data-capture-filter-bar]").count(), 1, "Mobile Queue should expose capture filters");
