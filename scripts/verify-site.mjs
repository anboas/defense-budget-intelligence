...
try {
  ...
  await page.waitForSelector("[data-capture-targeting]");
  assert.equal(await page.locator("[data-capture-chart]").count(), 19, "Capture Calendar should expose nineteen linked charts");
  assert.equal(await page.locator("[data-targeting-chart]").count(), 6, "Capture Calendar should expose six D3 targeting views");
  ...
  assert.ok(await page.locator("[data-targeting-chart='target-map'] .target-bubble").count() >= 20, "Target map should expose a substantial published-decision universe");
  assert.ok(await page.locator("[data-targeting-chart='target-route-map'] .target-route-node.is-record").count() >= 5, "Route map should connect ranked targets to published buyer and route evidence");
  assert.ok(await page.locator("[data-targeting-chart='buyer-portfolio-matrix'] .target-matrix__cell").count() >= 8, "Buyer-portfolio matrix should expose actionable concentration cells");
  assert.ok(await page.locator("[data-targeting-chart='incumbent-pack'] .target-pack-node.is-interactive").count() >= 3, "Incumbent map should expose selectable value concentrations");
  assert.ok(await page.locator("[data-targeting-chart='adjacency-map'] .target-adjacency__node").count() >= 3, "Adjacency map should expose related targeting paths");
  ...
  const targetPortfolio = await page.locator("[data-targeting-chart='target-treemap'] [role='button']").first().getAttribute("aria-label");
  await page.locator("[data-targeting-chart='target-treemap'] [role='button']").first().click();
  await page.waitForFunction(() => new URLSearchParams(window.location.hash.split("?")[1] || "").has("capPortfolio"));
  assert.equal(await page.locator(".capture-filter").filter({ hasText: /^Portfolio/ }).locator("select").inputValue(), targetPortfolio.split(",")[0], "Selecting a D3 treemap cell should drive the Gantt portfolio filter");
  await page.locator("[data-capture-filters]").getByRole("button", { name: "Reset" }).click();
  await page.locator("[data-targeting-chart='buyer-portfolio-matrix'] .target-matrix__cell").first().click();
  await page.waitForFunction(() => {
    const params = new URLSearchParams(window.location.hash.split("?")[1] || "");
    return params.has("capOffice") && params.has("capPortfolio");
  });
  await page.locator("[data-capture-filters]").getByRole("button", { name: "Reset" }).click();
  await page.locator("[data-targeting-chart='incumbent-pack'] .target-pack-node.is-interactive").first().click();
  await page.waitForFunction(() => new URLSearchParams(window.location.hash.split("?")[1] || "").has("capParty"));
  await page.locator("[data-capture-filters]").getByRole("button", { name: "Reset" }).click();
  const adjacentTargetId = (await page.locator("[data-targeting-chart='adjacency-map'] .target-adjacency__node").first().getAttribute("aria-label")).split(",")[0];
  await page.locator("[data-targeting-chart='adjacency-map'] .target-adjacency__node").first().click();
  await page.waitForFunction(() => new URLSearchParams(window.location.hash.split("?")[1] || "").has("capRecord"));
  assert.match(await page.locator("[data-capture-target-brief] h3").innerText(), new RegExp(`^${adjacentTargetId}\\b`), "Selecting an adjacency should refresh the target execution brief");
  ...
  const captureFilterPanel = page.locator("[data-capture-filters]");
  assert.equal(await captureFilterPanel.getByLabel("Funding / contracting office").inputValue(), "all", "Unknown office links should canonicalize to the declared default");
  assert.equal(await captureFilterPanel.getByLabel("Vehicle").inputValue(), "all", "Unknown vehicle links should canonicalize to the declared default");
  ...
  await mobile.waitForSelector("[data-capture-targeting]");
  ...
  const d3Scrollers = await mobile.locator(".target-d3-scroll").evaluateAll((elements) => elements.map((element) => ({ clientWidth: element.clientWidth, scrollWidth: element.scrollWidth })));
  assert.ok(d3Scrollers.every((box) => box.scrollWidth > box.clientWidth), `Mobile D3 charts should stay inside explicit internal scrollers: ${JSON.stringify(d3Scrollers)}`);
