...
try {
  ...
  assert.match(await aiLauncher.innerText(), /Background workflow[\s\S]*Research[\s\S]*Verify[\s\S]*Review/i, "Events must explain the detached research, verification, and notification review lifecycle");
  ...
  const aiAction = aiLauncher.getByRole("button", { name: "Start research and move on" });
  ...
  const actionSpinner = page.locator("[data-event-ai-launcher] .if-btn__spinner");
  ...
  const mobileAiGeometry = await mobileLauncher.evaluate((node) => {
    ...
    const action = node.querySelector(".if-btn--ai")?.getBoundingClientRect();
