...
try {
  ...
  assert.equal(await page.locator('[data-nav-group-trigger="platform-admin"] .ci-header-nav__menu-trigger-label').innerText(), "People & Access", "Global identity and workspace authority must remain visibly grouped");
  ...
  await page.waitForFunction(async () => {
    ...
    return body.data?.some((event) => event.catalogEventId === "catalog-spacecom-2027");
  });
  ...
  await page.goto(`${BASE_URL}#/budget-spend/schedule?scheduleView=list`, { waitUntil: "domcontentloaded" });
  await page.waitForSelector("[data-ops-events]");
  const augmentedEventRow = page.locator('[data-if-table-row]', { hasText: "SpaceCom / Space Congress 2027" });
  const augmentedEventAction = augmentedEventRow.getByRole("button", { name: /^Research and augment / });
  ...
  const augmentedEventBeforeReview = await page.evaluate(async (eventId) => {
    const response = await fetch("/api/v1/agent/events");
    const payload = await response.json();
    return payload.data?.find((event) => event.id === eventId) || null;
  }, augmentedEventId);
  assert.equal(augmentedEventBeforeReview?.startsAt, "2027-01-11T08:00", "The review regression requires a scheduled source event");
  assert.equal(augmentedEventBeforeReview?.endsAt, "2027-01-14T17:00", "The review regression requires the source event end date");
  ...
  await page.setViewportSize({ width: 1440, height: 1000 });
  assert.equal(await aiReview.getByRole("button", { name: "Save verified changes" }).count(), 1, "Reviewed event changes must expose a direct save path without reopening the record first");
  await aiReview.getByRole("button", { name: "Edit before saving" }).click();
  ...
  await page.getByText("Event changes saved", { exact: true }).waitFor();
  await page.waitForURL((url) => url.hash === `#/budget-spend/schedule?scheduleView=list&event=${encodeURIComponent(augmentedEventId)}`);
  await page.waitForSelector(`[data-ops-event-table] [data-row-key="${augmentedEventId}"][data-row-highlighted="true"]`);
  assert.equal(await page.locator("[data-event-ai-review]").count(), 0, "Saving a reviewed event must leave the stale Task Center changes page");
  assert.equal(await page.locator(`[data-row-key="${augmentedEventId}"]`).getAttribute("aria-current"), "true", "The saved event must be the focused Events row");
  assert.equal(await page.locator(`[data-row-key="${augmentedEventId}"] + [data-if-table-detail]`).count(), 1, "The saved event must reopen with its detail visible");
  await page.setViewportSize({ width: 390, height: 844 });
  ...
  assert.equal(augmentedEventState.aiValidationRequired, false, `Saving the reviewed draft must clear its validation state: ${JSON.stringify(augmentedEventState)}`);
  assert.ok(augmentedEventState.lastAiAppliedAt, `Saving the reviewed draft must retain its application timestamp: ${JSON.stringify(augmentedEventState)}`);
  ...
  await page.locator('[data-ops-event-table]').getByText(/Checked|AI amended/).first().waitFor({ timeout: 10_000 });
  assert.match(await page.locator('[data-ops-event-table]').innerText(), /(Checked|AI amended)[\s\S]*Last augmented/i, "The Events grid must expose the applied augmentation and recency without reopening Task Center");
  await page.setViewportSize({ width: 1440, height: 1000 });
  await page.goto(`${BASE_URL}#/budget-spend/connections?connectionsView=activity`, { waitUntil: "domcontentloaded" });
  ...
  const acquisitionSourceDetails = page.locator("[data-acquisition-coverage]");
  await acquisitionSourceDetails.waitFor();
  await acquisitionSourceDetails.locator(":scope > summary").click();
