...
try {
  ...
  await eventEditor.getByRole("button", { name: /^Research model: gpt-5\.4/ }).waitFor();
  await eventEditor.getByRole("button", { name: /^Verification model: gpt-5\.4/ }).waitFor();
  assert.match(await eventEditor.locator("[data-event-ai-assistant]").innerText(), /credential-specific[\s\S]*3 compatible text models/i, "The editor must disclose credential-specific model inventory semantics");
  const aiAction = eventEditor.getByRole("button", { name: "Research and augment" });
  ...
  assert.match(await eventEditor.getByRole("button", { name: /^Research model:/ }).getAttribute("aria-label"), /gpt-5\.4-mini/, "The research and verification models must be independently selectable");
  await eventEditor.locator("[data-event-ai-assistant]").scrollIntoViewIfNeeded();
  ...
  assert.match(await eventEditor.locator('[data-event-ai-job="completed"]').innerText(), /Research: gpt-5\.4-mini · Verification: gpt-5\.4/i, "The completed job must disclose the independently selected models");
  ...
  const mobileAiGeometry = await eventEditor.evaluate((node) => {
    ...
    const action = node.querySelector("[data-event-ai-assistant] .if-btn--ai")?.getBoundingClientRect();
    const direction = node.querySelector("[data-event-ai-assistant] textarea")?.getBoundingClientRect();
    const modelTriggers = [...node.querySelectorAll("[data-event-ai-model-picker] .if-picker__trigger")].map((trigger) => trigger.getBoundingClientRect());
    return { left: rect.left, right: rect.right, viewport: innerWidth, actionHeight: action?.height || 0, directionHeight: direction?.height || 0, modelHeights: modelTriggers.map((trigger) => trigger.height), modelContained: modelTriggers.every((trigger) => trigger.left >= 0 && trigger.right <= innerWidth + 1), documentOverflow: document.documentElement.scrollWidth - innerWidth };
  });
  ...
  assert.equal(mobileAiGeometry.modelHeights.length, 2, "The mobile event editor must expose both model pickers");
