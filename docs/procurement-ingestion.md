...
Authenticated workspaces also have an independent runtime SAM.gov source. A workspace manager stores a write-only encrypted key under Connections, then configures cadence, organization scope, notice types, lookback windows, page size, page ceiling, pacing, and retries in Spend Explorer. The hourly Cloudflare scheduler selects only due workspaces that have automation enabled and an active key. A workspace without a key creates no acquisition run and sends no source request. Runtime pulls honor `Retry-After`, use bounded exponential backoff, fail closed on truncated pagination, and retain the prior verified corpus on any source failure. Workspace records, observations, field changes, exact-identifier links, saved views, alerts, delivery preferences, delivery jobs, and bounded provider-attempt evidence remain workspace-scoped in D1/PostgreSQL. Immediate alerts and daily digests use a protected Resend adapter with idempotency keys, bounded exponential retry, terminal failed state, user opt-out, and Super-user retry/cancel controls; absent provider configuration never suppresses in-app alerts or discards jobs.
