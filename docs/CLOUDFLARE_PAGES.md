...
The hourly scheduler also drains due acquisition alert jobs. Immediate saved-view matches are eligible on the next scheduler invocation or immediately after a manual refresh; daily jobs wait for the configured UTC digest hour. The preferred provider path is the encrypted, write-only Resend vault under **Connections → Operations**, where the real Super user can replace or revoke the key, verify sender-domain readiness, and send a test only to their signed-in account. Protected `RESEND_API_KEY`, `DBI_ALERT_FROM_EMAIL`, and optional `DBI_ALERT_REPLY_TO` environment values remain a deployment fallback. `DBI_PUBLIC_APP_URL` and `DBI_ALERT_DIGEST_HOUR_UTC` remain deployment configuration. Never place the provider key in repository files, chat, shell arguments, or logs.

If the provider is absent, no email request is attempted and jobs remain `pending_provider`. In-app alerts remain the source of truth. Delivery attempts retain only status, bounded error code, provider message ID, and timestamp. The real Super user can inspect and retry or cancel jobs under **Connections → Operations**; emulated sessions cannot access that surface. The same surface retains deduplicated operational incidents and supports acknowledge, resolve, and reopen actions. New open incidents are emailed to active Super users once when a verified/configured provider is available.
